package _73_BSTIterator

// BSTIterator 173. 二叉搜索树迭代器
...
func Constructor(root *TreeNode) BSTIterator {
	...
	inorderFlatten(root, &slice)
	return BSTIterator{slice: slice}
}

func inorderFlatten(root *TreeNode, slice *[]*TreeNode) {
	if root == nil {
		return
	}
	inorderFlatten(root.Left, slice)
	*slice = append(*slice, root)
	inorderFlatten(root.Right, slice)
}

func (iter *BSTIterator) Next() int {
	result := iter.slice[0].Val

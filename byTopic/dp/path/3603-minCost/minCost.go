	...
	for i := 0; i < m; i++ {
		for j := 0; j < n; j++ {
			if i*j == 0 {
				...
				if i == 0 {
					dp[i][j] = dp[i][j-1] + int64(waitCost[i][j]+(i+1)*(j+1))
				} else {
					dp[i][j] = dp[i-1][j] + int64(waitCost[i][j]+(i+1)*(j+1))
				}
			} else {
				dp[i][j] = min(dp[i-1][j], dp[i][j-1]) + int64(waitCost[i][j]+(i+1)*(j+1))
			}
		}
	}

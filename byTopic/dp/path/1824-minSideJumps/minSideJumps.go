...
func minSideJumps(obstacles []int) int {
	...
	if obstacles[1] == 2 {
		...
		dp[0][0], dp[0][2] = math.MaxInt/2, math.MaxInt/2
	}
	for i := 1; i <= n; i++ {
		posVal := obstacles[i]
		if posVal > 0 {
			dp[i][posVal-1] = math.MaxInt / 2
			dp[i][(posVal)%3], dp[i][(posVal+1)%3] = dp[i-1][(posVal)%3]+1, dp[i-1][(posVal+1)%3]+1
			dp[i][(posVal)%3] = min(dp[i][(posVal)%3], dp[i][(posVal+1)%3]+1)
			dp[i][(posVal+1)%3] = min(dp[i][(posVal+1)%3], dp[i][(posVal)%3]+1)
		} else {
			dp[i][0], dp[i][1], dp[i][2] = dp[i-1][0]+1, dp[i-1][1]+1, dp[i-1][2]+1

...
func minPathCost(grid [][]int, moveCost [][]int) int {
	...
	for j, v := range grid[0] {
		dp[0][j] = &Node{val: v, sum: v}
	}
	for i := 1; i < m; i++ {
		for j := 0; j < n; j++ {
			currVal, currMin := grid[i][j], math.MaxInt
			for k := 0; k < n; k++ {
				preVal, preSum := dp[i-1][k].val, dp[i-1][k].sum
				cost := moveCost[preVal][j]
				currMin = min(currMin, preSum+cost+currVal)
			}
			dp[i][j] = &Node{val: currVal, sum: currMin}
		}
	}
	minPathSum := math.MaxInt
	for _, node := range dp[m-1] {
		minPathSum = min(minPathSum, node.sum)
	}
	return minPathSum
}

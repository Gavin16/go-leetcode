...
// 3418. 机器人可以获得的最大金币数
// 给你一个 m x n 的网格。一个机器人从网格的左上角 (0, 0) 出发，
// 目标是到达网格的右下角 (m - 1, n - 1)。在任意时刻，机器人只能向右或向下移动。
//
...
func maximumAmount(coins [][]int) int {
	...
	for i := range coins {
		...
		for j, v := range coins[i] {
			f[i+1][j+1][0] = max(f[i+1][j][0], f[i][j+1][0]) + v
			f[i+1][j+1][1] = max(f[i+1][j][0], f[i+1][j][1]+v, f[i][j+1][0], f[i][j+1][1]+v)
			f[i+1][j+1][2] = max(f[i+1][j][1], f[i+1][j][2]+v, f[i][j+1][1], f[i][j+1][2]+v)
		}
	}
	return f[m][n][2]
}

...
func securityCheck(capacities []int, k int) int {
	// 根据题设描述,问题可以转化为
	// 从capacities 中选择若干个安检室出来，用来作为FILO
	// 每个容量为 n 的FILO 结构可以让编号为k 的前n-1个元素被阻塞
	// 而对于FIFO结构,因为是顺序进出, 对阻塞的贡献为 0
	// 所以本题就是求 capacities 中元素值减去1 和为 k 的方案数
	dp := make([]int, k+1)
	dp[0] = 1
	MOD := int(1e9 + 7)
